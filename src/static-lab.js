...
const StaticLab=(()=>{
 ...
 function draw(){if(!p||!$('#elastic-apparatus'))return;const q=state(p),a=surface('elastic-apparatus'),{c,w,h,el}=a,{origin,scale}=geometry(w),cx=origin+q.x*100*scale,cy=h*.55;
  ...
  arrow(c,cx,cy-30,cx+q.F/2*w*.18,O);arrow(c,cx,cy+32,cx-q.kx/2*w*.18,B);
  c.textAlign='left';c.fillStyle=B;c.fillText('弹簧恢复力 −kx',5,h-7);c.textAlign='right';c.fillText(fmt(-q.kx)+' N',w-5,h-7);
  el.setAttribute('aria-valuenow',fmt(q.x*100));el.setAttribute('aria-valuetext',`位移${fmt(q.x*100)}厘米，外力${fmt(q.F)}牛顿`);
  $('#elastic-values').innerHTML=`<span>刚度 k<b>${p.stiffness} N/m</b></span><span>位移 x<b>${fmt(q.x*100)} cm</b></span><span>kx = F<b>${fmt(q.kx)} N</b></span>`;
  ...
  ctx.fillStyle='#536575';ctx.textAlign='left';ctx.fillText('F / N',4,13);ctx.textAlign='right';ctx.fillText('x / cm',W-3,H-3);
 ...
 function setup(params,changeForce){p=params;onForce=changeForce;const host=$('#compact-views');host.hidden=false;$('#experiment-tools').innerHTML='';$('#frequency-strip').innerHTML='<p class="static-intro">纯弹性静力 · <b>kx = F</b></p>';
  if(!$('#elastic-apparatus'))host.innerHTML='<section class="compact-panel elastic-model"><h3>拖动受力端 · 外力保持加载</h3><canvas id="elastic-apparatus" role="slider" tabindex="0" aria-label="拖动弹簧受力端，方向键调整外力" aria-valuemin="-10" aria-valuemax="10"></canvas><div id="elastic-values"></div><small>松手后保持平衡；虚线是未变形位置。</small></section><section class="compact-panel"><h3>外力—位移关系 · 直线斜率为刚度 k</h3><canvas id="elastic-curve" role="img" aria-label="外力位移直线，横轴负10至10厘米，纵轴负2至2牛顿"></canvas><small>改变 k 时保持 F，比较同一外力下的位移。</small></section>';
